...
class RandomBatchSamplerWithPosAndNeg(BatchSampler):
    def __init__(self, dataset, args, drop_last=False):
        ...
        self.dataIdx_set = set(range(len(dataset)))

        # 读取正负样本字典
        self.sample_dict = np.load(self.sample_dict, allow_pickle = True).item()
    ...
    def __iter__(self):
        ...
        for i in self.sampler:
            ...
            unknow_relation_list = list(self.dataIdx_set - set(self.sample_dict[i]['neg']) - set(self.sample_dict[i]['pos'])) # 未知关系的样本列表
